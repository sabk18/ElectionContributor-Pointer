...
os.chdir('/home/ubuntu/airflow/dags/Python_Modules')
...
def webtos3(ds, **kwargs):
    ...
    os.chdir('/home/ubuntu/airflow/dags')        
    #save all urls into a txt file        
    text_urls(all_urls)   
    
    path = '/home/ubuntu/airflow/dags/url_folder'
...
dag_start_date_utc = datetime.strptime('2020-06-24', '%Y-%m-%d')

args = {'owner': 'skhalid',
        'start_date': dag_start_date_utc,
        'email': ['saba.khalid1912@gmail.com'],
        'email_on_failure': True,
        'email_on_retry': True,
        'catchup': False,
        }

dag = DAG(dag_id='election_committee_v1',
          default_args=args,
          catchup=False,
          #schedule_interval='15 14 1 * *',
          max_active_runs=20)
          

webtos3 = PythonOperator(
task_id = 'webtos3',
provide_context=True,
python_callable = webtos3,
dag = dag)

s3_processing = PythonOperator(
task_id ='s3_processing',
provide_context =True,
python_callable = s3_processing,
dag = dag)

webtos3 >> s3_processing
